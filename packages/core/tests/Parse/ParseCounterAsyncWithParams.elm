module Parse.ParseCounterAsyncWithParams exposing (..)

import Elm.CodeGen exposing (intAnn, recordAnn, stringAnn, tupleAnn)
import ElmApp.Module as Module exposing (Init(..), Model(..), Msg(..), Params(..), Subscriptions(..), Update(..), View(..))
        ...
        [ test "with Msg -> Model -> (Model, Cmd Msg)" <|
            \_ ->
                let
                    ...
                    modelCmd =
                        tupleAnn
                            [ typeSimple "Model", typedConcreteSimple "Cmd" "Msg" ]
...
mainFile =
    """
module Main exposing (Model, Msg, Params, subscriptions, update, view, init)
